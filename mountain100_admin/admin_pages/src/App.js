...
import './App.css';
import { noticeDatas, souvAplDatas, souvProcDatas, souvCompDatas, certAplDatas, certCompDatas } from './Db.js';
import { useState, useEffect } from "react";

function App() {


  const [applyData, setAplData] = useState();
  ...
  return (
    ...
    <div className="App">
      <Routes>
        <Route path='/' element={
          <Home write = {write} applyData = {applyData.length} predData = {predData.length} compData = {compData.length} 
          aplCertData = {aplCertData.length} compCertData = {compCertData.length}> </Home>}>
