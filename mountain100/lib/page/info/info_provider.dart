...
final infoSearchSortedProvider = StateProvider<List<MountainModel>>((ref)=>ref.watch(mountainListProvider));
...
class InfoPageProvider with InfoPageProviderInterface {
  ...
  filterByKeyword(String keyword) {
    ...
    if (keyword!=""){
      for (MountainModel model in ref.watch(mountainListProvider.notifier).state){
      ...
      ref.read(infoSearchSortedProvider.notifier).state=ref.watch(mountainListProvider.notifier).state;
    }
